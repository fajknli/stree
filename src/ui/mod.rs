...
pub fn render_all<W: Write>(ctx: &mut RenderCtx, all_rects: &[(WindowRect, String, BorderStyle, usize)], out: &mut W) -> std::io::Result<()> {
    ...
    PREV_BUFFER.with(|prev_cell| -> std::io::Result<()> {
        ...
        let mut curr_buffer = Buffer::empty(term_width, term_height);
        ...
        // 5. 将当前帧保存为下一帧的 prev (直接移动所有权，零拷贝)
        *prev_opt = Some(curr_buffer);
    ...
    for (r, n, _, _) in all_rects { current_rects_map.insert(n.clone(), *r); }
